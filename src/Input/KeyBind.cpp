...
namespace obe
{
    namespace Input
    {
        ...
        KeyBinder::KeyBinder() : m_keysTriggers(Triggers::TriggerDatabase::GetInstance()->createTriggerGroup("Global", "Keys"))
        {
        ...
        void KeyBinder::update()
        {
            if (m_binderEnabled)
            {
                ...
                for (it_type iterator = m_actionMap.begin(); iterator != m_actionMap.end(); ++iterator)
                {
                    ...
                    {
                        if (Utils::Map::keyInMap(iterator->first, m_actionDelayer))
                        {
        ...
        bool KeyBinder::isActionEnabled(std::string action)
        {
            ...
            {
                if (Utils::Map::keyInMap(action, m_actionDelayer))
                {
