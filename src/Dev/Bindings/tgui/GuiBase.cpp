...
namespace tgui::Bindings
{
    void LoadClassGuiBase(sol::state_view state)
    {
        ...
        bindGuiBase["add"] = sol::overload(
            [](tgui::GuiBase* self, const Widget::Ptr& widgetPtr) -> void {
                return self->add(widgetPtr);
            },
            [](tgui::GuiBase* self, const Widget::Ptr& widgetPtr,
                const tgui::String& widgetName) -> void {
                return self->add(widgetPtr, widgetName);
            });
        bindGuiBase["get"] = sol::overload(
        ...
        bindGuiBase["loadWidgetsFromStream"] = static_cast<void (tgui::GuiBase::*)(std::stringstream&, bool)>(
