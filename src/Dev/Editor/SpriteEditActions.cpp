...
namespace obe::Editor
{
                                   ...
                                   Scene::Scene& scene, int& currentLayer)
    {
        ...
        inputManager.getAction("SpriteLayerInc")
            .connect([editorTriggers, &selectedSprite, &scene,
                      &currentLayer](const Input::InputActionEvent& event) {
                if (selectedSprite != nullptr)
                {
                    selectedSprite->setLayer(selectedSprite->getLayer() + 1);

                    editorTriggers->pushParameter("SpriteLayerChanged", "layer",
                                                  selectedSprite->getLayer());
                    editorTriggers->pushParameter("SpriteLayerChanged",
                                                  "sprite", selectedSprite);
                    editorTriggers->pushParameter("SpriteLayerChanged",
                                                  "operation", "Increase");
    ...
    connectSpriteActions(Triggers::TriggerGroup* editorTriggers,
                         ...
                         Editor::Tooltip& tooltip, Transform::Units& editorUnit)
    {
        inputManager.getAction("MoveHandlePoint")
            .connect([editorTriggers, &selectedHandlePoint, &cursor,
                      &world](const Input::InputActionEvent& event) {
                ...
                {
                    selectedHandlePoint->moveTo(cursor.getPosition(),
                                                pixelCamera);
                    editorTriggers->pushParameter("SpriteHandlePointMoved",
                                                  "handlePoint",
                                                  selectedHandlePoint);
        ...
        inputManager.getAction("ReleaseHandlePoint")
            .connect([editorTriggers, &selectedHandlePoint](
                         const Input::InputActionEvent& event) {
                if (selectedHandlePoint != nullptr)
                {
                    editorTriggers->pushParameter("SpriteHandlePointReleased",
                                                  "handlePoint",
                                                  selectedHandlePoint);
        ...
        inputManager.getAction("SpritePick")
            .connect([editorTriggers, &selectedSprite, &selectedSpriteOffsetX,
                      &selectedSpriteOffsetY, &cursor, &selectedHandlePoint,
                      &hoveredSprite, &tooltip,
                      &world](const Input::InputActionEvent& event) {
                Transform::UnitVector pixelCamera =
                    world.getCamera()
                        ->getPosition()
                ...
                {
                    selectedHandlePoint = selectedSprite->getHandlePoint(
                        pixelCamera, cursor.getX(), cursor.getY());
                    if (selectedHandlePoint != nullptr)
                    {
                        editorTriggers->pushParameter("SpriteHandlePointPicked",
                                                      "handlePoint",
                                                      selectedHandlePoint);
                        editorTriggers->pushParameter(
                            "SpriteHandlePointPicked", "pos",
                    ...
                    {
                        editorTriggers->pushParameter("SpriteUnselect",
                                                      "sprite", selectedSprite);
                ...
                if (hoveredSprite != nullptr && selectedHandlePoint == nullptr)
                {
                    ...
                    editorTriggers->pushParameter("SpriteSelect", "sprite",
                                                  selectedSprite);
                    editorTriggers->pushParameter(
                        "SpriteSelect", "offset",
                        Transform::UnitVector(selectedSpriteOffsetX,
                                              selectedSpriteOffsetY,
                                              Transform::Units::ScenePixels));
                    editorTriggers->pushParameter(
                        "SpriteSelect", "pos",
                        pixelCamera + Transform::UnitVector(
        ...
        inputManager.getAction("SpriteMove")
            .connect([editorTriggers, &selectedSprite, &cursor,
                      &selectedSpriteOffsetX, &selectedSpriteOffsetY,
                      &selectedHandlePoint, &tooltip, &world,
                      &editorUnit](const Input::InputActionEvent& event) {
                if (selectedSprite != nullptr && selectedHandlePoint == nullptr)
                {
                    editorTriggers->pushParameter("SpriteMoved", "sprite",
                                                  selectedSprite);
                    editorTriggers->pushParameter(
                    ...
                    Transform::UnitVector pixelCamera =
                        world.getCamera()
                            ->getPosition()
                            .to<Transform::Units::ScenePixels>();
                    Transform::UnitVector basePos(
                        cursor.getConstrainedX() - selectedSpriteOffsetX,
                        cursor.getConstrainedY() - selectedSpriteOffsetY,
                        Transform::Units::ScenePixels);
                    Transform::UnitVector newPosition =
                        selectedSprite->getPositionTransformer()(
                    ...
                    tooltip.setText(
                        ...
                        selectedSprite->getSize().to(editorUnit).y,
                        selectedSprite->getRotation(),
                        selectedSprite->getLayer(),
                        selectedSprite
                            ->getZDepth()); // <REVISION> Duplicated code (See
                                            // MapEditor.cpp)
        ...
        inputManager.getAction("SpriteRemove")
            .connect(
                [editorTriggers, &selectedSprite, &world, &tooltip,
                 &hoveredSprite, &selectedSpriteOffsetX,
                 &selectedSpriteOffsetY](const Input::InputActionEvent& event) {
                    if (selectedSprite != nullptr)
                    {
                        editorTriggers->pushParameter("SpriteRemoved", "id",
        ...
        inputManager.getAction("SpriteCancelOffset")
            .connect([&selectedSprite, &cursor, &editorGrid,
                      &selectedSpriteOffsetX, &selectedSpriteOffsetY,
                      &selectedHandlePoint](
                         const Input::InputActionEvent& event) {
                if (selectedSprite != nullptr && selectedHandlePoint == nullptr)
