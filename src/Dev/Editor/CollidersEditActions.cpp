...
namespace obe::Editor
{
        ...
        bool& masterColliderGrabbed)
    {
        inputManager.getAction("ColliderPointPick")
            .connect([editorTriggers, &scene, &cursor, &selectedMasterCollider,
                      &masterColliderGrabbed, &colliderPtGrabbed](
                ...
                if (colliderPtGrabbed == -1 &&
                    scene.getColliderPointByPosition(cursCoord).first !=
                        nullptr)
                {
                    std::pair<Collision::PolygonalCollider*, int>
                        selectedPtCollider;
                    selectedPtCollider =
                        scene.getColliderPointByPosition(cursCoord);
                    ...
                    editorTriggers->pushParameter("ColliderPointPicked",
                                                  "collider",
                                                  selectedMasterCollider);
                    editorTriggers->pushParameter(
                        "ColliderPointPicked", "pointIndex", colliderPtGrabbed);
                    editorTriggers->pushParameter("ColliderPointPicked", "pos",
                                                  cursCoord);
        ...
        inputManager.getAction("ColliderPointMove")
            .connect([editorTriggers, &selectedMasterCollider,
                      &masterColliderGrabbed, &colliderPtGrabbed, &scene,
                      &cursor](const Input::InputActionEvent& event) {
                Debug::Log->debug(
                    "Action <ColliderPointMoved (pre)> triggered");
                if (selectedMasterCollider != nullptr &&
                    !masterColliderGrabbed && colliderPtGrabbed != -1)
                {
                    Transform::UnitVector pixelCamera =
                        scene.getCamera()
                            ->getPosition()
                            .to<Transform::Units::ScenePixels>();
                    Transform::UnitVector cursCoord(
                        cursor.getConstrainedX() + pixelCamera.x,
                        cursor.getConstrainedY() + pixelCamera.y,
                        Transform::Units::ScenePixels);
                    editorTriggers->pushParameter("ColliderPointMoved",
                                                  "collider",
                                                  selectedMasterCollider);
                    editorTriggers->pushParameter(
                        "ColliderPointMoved", "pointIndex", colliderPtGrabbed);
                    editorTriggers->pushParameter(
                        "ColliderPointMoved", "oldPos",
                        selectedMasterCollider->get(colliderPtGrabbed)
                            .to<Transform::Units::ScenePixels>());
                    editorTriggers->pushParameter("ColliderPointMoved", "pos",
                                                  cursCoord);
        ...
        inputManager.getAction("ColliderPointRelease")
            .connect([editorTriggers, &colliderPtGrabbed,
                      &selectedMasterCollider](
                         const Input::InputActionEvent& event) {
                editorTriggers->pushParameter("ColliderPointReleased",
                                              "collider",
                                              selectedMasterCollider);
                editorTriggers->pushParameter("ColliderPointReleased",
                                              "pointIndex", colliderPtGrabbed);
        ...
        inputManager.getAction("ColliderMasterPick")
            .connect([editorTriggers, &scene, &cursor, &selectedMasterCollider,
                      &colliderPtGrabbed, &masterColliderGrabbed](
                         const Input::InputActionEvent& event) {
                Transform::UnitVector pixelCamera =
                    scene.getCamera()
                        ->getPosition()
                ...
                if (scene.getColliderByCentroidPosition(cursCoord) != nullptr)
                {
                    ...
                    if (selectedMasterCollider->getParentId() != "")
                        scene
                            .getGameObject(
                                selectedMasterCollider->getParentId())
                            ->setUpdateState(false);
                    masterColliderGrabbed = true;
                    editorTriggers->pushParameter("ColliderPicked", "collider",
                                                  selectedMasterCollider);
                    editorTriggers->pushParameter("ColliderPicked", "pos",
                                                  cursCoord);
            ...
            .connect([editorTriggers, &selectedMasterCollider,
                ...
                if (selectedMasterCollider != nullptr && masterColliderGrabbed)
                {
                    ...
                    Transform::UnitVector cursCoord(
                        ...
                        cursor.getConstrainedY() + pixelCamera.y,
                        Transform::Units::ScenePixels);
                    editorTriggers->pushParameter("ColliderMoved", "collider",
                                                  selectedMasterCollider);
                    editorTriggers->pushParameter(
                        "ColliderMoved", "oldPos",
                        selectedMasterCollider->getCentroid()
                            .to<Transform::Units::ScenePixels>());
                    editorTriggers->pushParameter("ColliderMoved", "pos",
                                                  cursCoord);
        ...
        inputManager.getAction("ColliderMasterRelease")
            .connect([editorTriggers, &masterColliderGrabbed,
                      &selectedMasterCollider,
                      &scene](const Input::InputActionEvent& event) {
            ...
            .connect([editorTriggers, &selectedMasterCollider,
                ...
                if (selectedMasterCollider != nullptr && !masterColliderGrabbed)
                {
                    ...
                    if (!selectedMasterCollider
                             ->getPointAroundPosition(cursCoord, pTolerance)
                             .has_value())
                    {
                        auto newSegment =
                            selectedMasterCollider->findClosestSegment(
                                cursCoord);
                        editorTriggers->pushParameter("ColliderPointCreated",
                                                      "collider",
                                                      selectedMasterCollider);
                        editorTriggers->pushParameter("ColliderPointCreated",
                                                      "pointIndex",
                                                      newSegment.first.index);
                        editorTriggers->pushParameter("ColliderPointCreated",
                                                      "pos", cursCoord);
                        editorTriggers->trigger("ColliderPointCreated");
                        selectedMasterCollider->addPoint(
                            cursCoord, newSegment.first.index);
                    }
                }
            });
        ...
        inputManager.getAction("ColliderPointRemove")
            .connect([editorTriggers, &selectedMasterCollider,
                    ...
                    !masterColliderGrabbed && colliderPtGrabbed != -1)
                {
                    ...
                    {
                        editorTriggers->pushParameter("ColliderPointRemoved",
                                                      "collider",
                                                      selectedMasterCollider);
                        editorTriggers->pushParameter("ColliderPointRemoved",
                                                      "pointIndex",
            ...
            .connect([editorTriggers, &selectedMasterCollider, &scene, &cursor,
                ...
                if (selectedMasterCollider != nullptr)
                {
                    ...
                    Transform::UnitVector cursCoord(
                        ...
                        cursor.getConstrainedY() + pixelCamera.y,
                        Transform::Units::ScenePixels);
                    if (scene.getColliderByCentroidPosition(cursCoord) ==
                        nullptr)
                    {
                        if (scene.getColliderPointByPosition(cursCoord).first ==
                            nullptr)
                        {
                            editorTriggers->pushParameter(
                                "ColliderReleased", "collider",
                                selectedMasterCollider);
                            editorTriggers->pushParameter("ColliderReleased",
                                                          "pos", cursCoord);
                            editorTriggers->trigger("ColliderReleased");
                            selectedMasterCollider->setSelected(false);
                            selectedMasterCollider = nullptr;
        ...
        inputManager.getAction("ColliderRemove")
            .connect([editorTriggers, &selectedMasterCollider,
                      &masterColliderGrabbed, &scene, &colliderPtGrabbed](
                         const Input::InputActionEvent& event) {
                if (selectedMasterCollider != nullptr && masterColliderGrabbed)
                {
                    editorTriggers->pushParameter(
        ...
        inputManager.getAction("ColliderCreate")
            .connect([editorTriggers, &selectedMasterCollider, &scene,
                      &cursor](const Input::InputActionEvent& event) {
                if (selectedMasterCollider == nullptr)
                {
                    Transform::UnitVector pixelCamera =
                        scene.getCamera()
                            ->getPosition()
                            .to<Transform::Units::ScenePixels>();
                    Transform::UnitVector pPos(cursor.getConstrainedX(),
                                               cursor.getConstrainedY(),
                                               Transform::Units::ScenePixels);
                    pPos += pixelCamera;
                    Collision::PolygonalCollider* newCollider =
                        scene.createCollider();
                    newCollider->addPoint(Transform::UnitVector(
                        50, 0, Transform::Units::ScenePixels));
                    newCollider->addPoint(Transform::UnitVector(
                        0, 50, Transform::Units::ScenePixels));
                    newCollider->addPoint(Transform::UnitVector(
                        100, 50, Transform::Units::ScenePixels));
                    newCollider->setPositionFromCentroid(pPos);
                    editorTriggers->pushParameter("ColliderCreated", "collider",
                                                  newCollider);
                    editorTriggers->pushParameter("ColliderCreated", "pos",
                                                  pPos);
