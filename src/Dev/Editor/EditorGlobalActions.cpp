...
namespace obe::Editor
{
    void connectCamMovementActions(Triggers::TriggerGroup* editorTriggers,
                                   ...
                                   Scene::Scene& world, int& cameraSpeed,
                                   Time::FramerateManager& framerateManager)
    {
        inputManager.getAction("CamLeft").connect(
            [editorTriggers, &world, &cameraSpeed,
             &framerateManager](const Input::InputActionEvent& event) {
                ...
                editorTriggers->pushParameter("CameraMoved", "direction",
                                              "Left");
                editorTriggers->pushParameter("CameraMoved", "move", moveVec);
                editorTriggers->pushParameter("CameraMoved", "camera",
                                              world.getCamera());
                editorTriggers->trigger("CameraMoved");
            });
        inputManager.getAction("CamRight")
            .connect([editorTriggers, &world, &cameraSpeed,
                      &framerateManager](const Input::InputActionEvent& event) {
                Transform::UnitVector moveVec(
                    cameraSpeed * framerateManager.getGameSpeed(), 0,
                    Transform::Units::ScenePixels);
        ...
        inputManager.getAction("CamUp").connect(
            [editorTriggers, &world, &cameraSpeed,
             &framerateManager](const Input::InputActionEvent& event) {
                Transform::UnitVector moveVec(
                    0, -cameraSpeed * framerateManager.getGameSpeed(),
                    Transform::Units::ScenePixels);
                world.getCamera()->move(moveVec);
                editorTriggers->pushParameter("CameraMoved", "direction",
                                              "Left");
                            ...
                            Editor::EditorGrid& editorGrid)
    {
        inputManager.getAction("MagnetizeUp")
            .connect([editorTriggers, enableGridCheckbox, &cursor,
                      &editorGrid](const Input::InputActionEvent& event) {
                if (enableGridCheckbox->isChecked())
                {
                    editorGrid.moveMagnet(cursor, 0, -1);
                    editorTriggers->pushParameter("GridCursorMoved",
                                                  "direction", "Up");
                    editorTriggers->pushParameter("GridCursorMoved", "grid",
                                                  editorGrid);
                    editorTriggers->pushParameter("GridCursorMoved", "cursor",
                                                  &cursor);
                    editorTriggers->trigger("GridCursorMoved");
                    editorTriggers->pushParameter("CursorMagnetized", "grid",
                                                  editorGrid);
                    editorTriggers->pushParameter("CursorMagnetized", "cursor",
                                                  &cursor);
        ...
        inputManager.getAction("ToggleGridSnap")
            .connect([editorTriggers,
                      snapGridCheckbox](const Input::InputActionEvent& event) {
                if (snapGridCheckbox->isEnabled())
                {
                    editorTriggers->pushParameter(
                        "GridSnapToggled", "state",
                        snapGridCheckbox->isChecked());
                            ...
                            tgui::CheckBox::Ptr saveCameraPositionCheckbox)
    {
        inputManager.getAction("Save").connect(
            [&mapName, &scene, &waitForMapSaving, savedLabel,
        ...
        Graphics::LevelSprite*& selectedSprite)
    {
             ...
             &selectedSprite](const Input::InputActionEvent& event) {
                ...
                if (selectedSprite)
                {
                    ...
                    sceneClipboard.at(selectedSprite->getId())
                        .createDataNode("type", "LevelSprite");
                }
            });

        inputManager.getAction("Cut").connect(
            [](const Input::InputActionEvent& event) {
        ...
        inputManager.getAction("Paste").connect(
            [&sceneClipboard, &scene](const Input::InputActionEvent& event) {
                ...
                for (vili::ComplexNode* item :
                     sceneClipboard.getAll<vili::ComplexNode>())
                {
                    if (item->at<vili::DataNode>("type").get<std::string>() ==
                        "Collider")
                    {
                        scene.createCollider()->load(*item);
                    }
                    else if (item->at<vili::DataNode>("type")
                                 .get<std::string>() == "LevelSprite")
                    {
                                   ...
                                   Debug::Console& gameConsole)
    {
        inputManager.getAction("ConsoleToggle")
            .connect([&gameConsole,
                      &inputManager](const Input::InputActionEvent& event) {
