...
namespace obe::Editor
{
    void connectSceneNodeActions(Triggers::TriggerGroup* editorTriggers,
                                 ...
                                 Scene::Scene& scene, System::Cursor& cursor,
                                 Scene::SceneNode*& sceneNodeGrabbed)
    {
        inputManager.getAction("SceneNodePick")
            .connect([editorTriggers, &scene, &cursor,
                      &sceneNodeGrabbed](const Input::InputActionEvent& event) {
                ...
                if (auto sceneNode = scene.getSceneNodeByPosition(cursCoord))
                {
                    ...
                    editorTriggers->pushParameter(
                        "SceneNodePicked", "sceneNode", sceneNodeGrabbed);
                    editorTriggers->pushParameter("SceneNodePicked", "pos",
                                                  cursCoord);
    ...
    inputManager.getAction("SceneNodeMove").connect(
        [editorTriggers, &sceneNodeGrabbed, &scene, &cursor](const Input::InputActionEvent& event) {
        if (sceneNodeGrabbed)
        {
            Transform::UnitVector pixelCamera =
                scene.getCamera()
            ...
            Transform::UnitVector cursCoord(
                ...
                Transform::Units::ScenePixels);
            editorTriggers->pushParameter("SceneNodeMoved", "sceneNode",
                                            sceneNodeGrabbed);
            editorTriggers->pushParameter(
                "SceneNodeMoved", "oldPos",
                sceneNodeGrabbed->getPosition()
                    .to<Transform::Units::ScenePixels>());
            editorTriggers->pushParameter("SceneNodeMoved", "pos",
                                            cursCoord);
        ...
        inputManager.getAction("SceneNodeRelease").connect(
