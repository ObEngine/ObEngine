...
namespace obe::Editor
{
    ...
    {
        System::Path("Data/Fonts/arial.ttf")
            .load(System::Loaders::fontLoader, m_font);
    ...
    sf::Texture* Thumbnailer::GetSpriteThumbnail(const std::string& path)
    {
        ...
        m_instance->m_cache[path] =
            new sf::Texture(m_instance->m_renderer.getTexture());
                        ...
                        tgui::Scrollbar::Ptr objectsScrollbar)
    {
        ...
        const auto getBtnPos = [&btnSize, &btnOff, &xOff, &panelWidth, &yOff,
            ...
            unsigned int ixPos =
                (index % maxElementsPerRow) * (btnSize + btnOff) + xOff;
            unsigned int iyPos =
                index / maxElementsPerRow * (btnSize + btnOff) + yOff;
        ...
        for (unsigned int i = 0; i < allGameObjects.size(); i++)
        {
            ...
            currentObj->connect("pressed", [&scene, &requiresPanel, &baseTheme,
                                            currentObjName]() {
                buildRequiresObjectTab(scene, requiresPanel, baseTheme,
                                       currentObjName);
            });
        }
                                ...
                                const std::string& objName)
    {
        ...
        if (requires != nullptr)
        {
            ...
            std::unordered_map<std::string, tgui::ComboBox::Ptr>
                requireComboBoxes;
            std::unordered_map<std::string, tgui::EditBox::Ptr>
                requireEditBoxes;
                 ...
                 requireInput.getAll<vili::ComplexNode>())
            {
                ...
                tgui::Label::Ptr currentRequirementLabel =
                    tgui::Label::create();
                currentRequirementLabel->setPosition(50,
                                                     widgetVerticalPosition);
                currentRequirementLabel->setTextSize(18);
                currentRequirementLabel->setRenderer(
                    baseTheme.getRenderer("Label"));
                ...
                {
                    tgui::EditBox::Ptr currentRequirementInput =
                        tgui::EditBox::create();
                    currentRequirementInput->setRenderer(
                        baseTheme.getRenderer("TextBox"));
                    currentRequirementInput->setSize("33%", "32");
                    currentRequirementInput->setPosition(
                        200, widgetVerticalPosition + 5);
                    content->add(currentRequirementInput,
                                 requireItem + "_input");
                ...
                {
                    tgui::ComboBox::Ptr currentRequirementList =
                        tgui::ComboBox::create();
                    currentRequirementList->setSize(200, 32);
                    currentRequirementList->setPosition(200,
                                                        widgetVerticalPosition);
                    ...
                    content->add(currentRequirementList,
                                 requireItem + "_input");
                    for (int reqI = 0; reqI < requireInput.at(requireItem)
                                                  .getArrayNode("choices")
                                                  .size();
                         reqI++)
                        currentRequirementList->addItem(
                            requireInput.at(requireItem)
                                .getArrayNode("choices")
                                .get(reqI)
                                .get<std::string>());
                ...
                "pressed", [objName, requireComboBoxes, requireEditBoxes, key,
                    ...
                    for (auto& cReq : requireComboBoxes)
                    {
                        requireCopy->at("Input", cReq.first)
                            .createDataNode("value",
                                            cReq.second->getSelectedItem());
                    }
        ...
        {
            Script::GameObject* newGameObject =
                scene.createGameObject(objName, key);
                                   ...
                                   vili::ComplexNode* requires)
    {
        ...
        Script::GameObjectDatabase::ApplyRequirements(newGameObject,
                                                      requires->at("Output"));
                          ...
                          tgui::Scrollbar::Ptr spritesScrollbar)
    {
        ...
        const auto getSpritePos = [&sprSize, &sprOff, &xOff, &panelWidth, &yOff,
                                   &maxElementsPerRow](unsigned int& index) {
            unsigned int ixPos =
                (index % maxElementsPerRow) * (sprSize + sprOff) + xOff;
            unsigned int iyPos =
                index / maxElementsPerRow * (sprSize + sprOff) + yOff;
            index++;
        ...
        for (std::string element : folderList)
        {
            ...
            currentFolder->connect("pressed", [spritesPanel, spritesCatLabel,
    ...
    void addSpriteToScene(Scene::Scene& scene, const std::string& spritePath)
    {
        int i = 0;
        std::string testId =
            "sprite" + std::to_string(scene.getLevelSpriteAmount() + i);
