...
namespace obe::Triggers
{
    ...
    Trigger::Trigger(TriggerGroup* parent, const std::string& name,
                     bool startState)
    {
        m_name = name;
        m_parent = parent;
        ...
        m_fullName =
            this->getNamespace() + "." + this->getGroup() + "." + m_name;
        Script::ScriptEngine["LuaCore"]["TriggerArgTable"]
                            [this->getTriggerLuaTableName()] =
                                kaguya::NewTable();
        Debug::Log->trace("<Trigger> Creating Trigger {0} @{1}", m_fullName,
                          fmt::ptr(this));
    }
    ...
    void Trigger::registerEnvironment(unsigned int envIndex,
                                      ...
                                      bool* envActive)
    {
        ...
        {
            Debug::Log->trace(
                "<Trigger> Calling onRegister callback of Trigger {0}",
                m_fullName);
    ...
    void Trigger::execute()
    {
        ...
        if (!m_envsToRemove.empty())
        {
            ...
            {
                m_registeredEnvs.erase(
                    std::remove_if(m_registeredEnvs.begin(),
                                   m_registeredEnvs.end(),
                                   [&envToRemove](TriggerEnv& env) {
                                       return env.envIndex == envToRemove;
                                   }),
                    m_registeredEnvs.end());
            }
        }
    ...
    void Trigger::pushParameterFromLua(const std::string& name,
                                       const kaguya::LuaRef& parameter) const
    {
        Debug::Log->trace(
            "<Trigger> Pushing parameter {0} to Trigger {1} (From Lua)", name,
            m_fullName);
        Script::ScriptEngine["LuaCore"]["TriggerArgTable"]
    ...
    {
        Debug::Log->trace("<Trigger> Add onRegister callback to Trigger {0}",
                          m_fullName);
    ...
    {
        Debug::Log->trace("<Trigger> Add onUnregister callback to Trigger {0}",
                          m_fullName);
