...
namespace obe::System::Package
{
    std::string GetPackageLocation(const std::string& packageName)
    {
        ...
        throw aube::ErrorHandler::Raise(
            "ObEngine.System.Package.InexistantPackage",
    ...
    bool Install(const std::string& packageName)
    {
        ...
        if (!Utils::Vector::contains(packageName + ".opaque",
                                     Utils::File::getFileList("Package")))
            throw aube::ErrorHandler::Raise(
                "ObEngine.System.Package.CantFindPackage",
                {{"package", packageName}});
        if (!PackageExists(packageName))
        {
            elz::extractFile("Package/" + packageName + ".opaque",
                             "Opaque.vili", "Package/Opaque.vili");
            ...
            const std::string packageVersion =
                cPackage.at<vili::DataNode>("Meta", "version")
                    .get<std::string>();
            elz::extractZip("Package/" + packageName + ".opaque",
                            "Package/" + realPackageName);
            Debug::Log->info(
                "<Package> Package '{0}' has been successfully installed",
                packageName);

            vili::ViliParser packages("Package/Packages.vili");
            packages->createComplexNode(realPackageName);
            packages.at(realPackageName)
                .createDataNode("version", packageVersion);
            packages.at(realPackageName)
        ...
        throw aube::ErrorHandler::Raise(
            "ObEngine.System.Package.AlreadyInstalled",
            {{"package", packageName}});
    }

    bool Load(const std::string& packageName, const unsigned int priority)
    {
        Debug::Log->info("<Package> Loading Package '{0}' with priority",
                         packageName, priority);
        if (PackageExists(packageName))
        {
            Path::Mount(MountablePath(MountablePathType::Package,
