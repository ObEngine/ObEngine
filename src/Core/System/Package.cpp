...
namespace obe::System::Package
{
    ...
    bool Install(const std::string& packageName)
    {
        ...
        if (!PackageExists(packageName))
        {
            ...
            const std::string realPackageName = cPackage.at<vili::DataNode>("Meta", "name").get<std::string>();
            const std::string packageVersion = cPackage.at<vili::DataNode>("Meta", "version").get<std::string>();
