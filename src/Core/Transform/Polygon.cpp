...
namespace obe::Transform
{
    PolygonPoint::PolygonPoint(Polygon* parent, unsigned index)
        : m_parent(parent), rw_index(index)
    {
    }
    ...
    UnitVector
    PolygonPoint::getRelativePosition(RelativePositionFrom from) const
    {
    ...
    void
    PolygonPoint::setRelativePosition(RelativePositionFrom from,
                                      const Transform::UnitVector& position)
    {
        if (from == RelativePositionFrom::Point0)
            this->set(position.to<Transform::Units::SceneUnits>() +
                      m_parent->get(0));
    ...
    PolygonSegment::PolygonSegment(const PolygonPoint& first,
    ...
    void Polygon::addPoint(const Transform::UnitVector& position,
                           int pointIndex)
    {
        const Transform::UnitVector pVec =
            position.to<Transform::Units::SceneUnits>();
    ...
    PolygonPoint&
    Polygon::findClosestPoint(const Transform::UnitVector& position,
                              bool neighbor,
        ...
        if (!m_points.empty())
        {
            ...
            if (neighbor)
            {
                ...
                if (rightNeighbor >= m_points.size())
                    rightNeighbor = 0;
                const int leftNeighborDist =
                    m_points[leftNeighbor]->distance(position);
                const int rightNeighborDist =
                    m_points[rightNeighbor]->distance(position);
    ...
    PolygonSegment
    Polygon::findClosestSegment(const Transform::UnitVector& position)
    {
        const Transform::UnitVector p3 =
            position.to<Transform::Units::SceneUnits>();
            ...
            [](const Transform::UnitVector& point,
               ...
               const Transform::UnitVector& lineP2) {
                ...
                if (lineDiff.x == 0 && lineDiff.y == 0)
                {
                    ...
                    return sqrt(lineDiff.x * lineDiff.x +
                                lineDiff.y * lineDiff.y);
                }

                double t = ((point.x - lineP1.x) * lineDiff.x +
                            (point.y - lineP1.y) * lineDiff.y) /
        ...
        for (unsigned int i = 0, j = getAllPoints().size() - 1;
             i < getAllPoints().size(); j = i++)
        {
    ...
    Polygon::getSegmentContainingPoint(const Transform::UnitVector& position,
                                       const double tolerance)
    {
        for (point_index_t i = 0; i < m_points.size(); i++)
        {
            const point_index_t nextNode =
                (i != m_points.size() - 1) ? i + 1 : 0;
            const double lineLength =
                m_points[i]->distance(this->get(nextNode));
            const double firstLength = m_points[i]->distance(position);
            const double secondLength = m_points[nextNode]->distance(position);
            if (Utils::Math::isBetween(lineLength,
    ...
    Polygon::getPointAroundPosition(const Transform::UnitVector& position,
                                    const Transform::UnitVector& tolerance)
    {
        ...
        {
            if (Utils::Math::isBetween(pVec.x, m_point->x - pTolerance.x,
                                       m_point->x + pTolerance.x))
            {
                if (Utils::Math::isBetween(pVec.y, m_point->y - pTolerance.y,
                                           m_point->y + pTolerance.y))
        ...
        const Transform::UnitVector& tolerance) const
    {
        ...
        if (Utils::Math::isBetween(pVec.x, centroid.x - pTolerance.x,
                                   centroid.x + pTolerance.x))
        {
            if (Utils::Math::isBetween(pVec.y, centroid.y - pTolerance.x,
                                       centroid.y + pTolerance.y))
                return true;
        }
        return false;
    }
    ...
    void Polygon::setRotation(const float angle,
                              const Transform::UnitVector origin)
    {
    ...
    void Polygon::rotate(float angle, Transform::UnitVector origin)
    {
        ...
        {
            point->set(
                std::cos(radAngle) * (point->x - origin.x) -
    ...
    PolygonPoint& Polygon::get(point_index_t i)
    {
        if (i < m_points.size())
            ...
            throw aube::ErrorHandler::Raise(
                "obe.Transform.Polygon.IndexOverflow",
