...
namespace obe::Config::Templates::Bindings
{
    ...
    {
        sol::table TemplatesNamespace = state["obe"]["Config"]["Templates"].get<sol::table>();
        TemplatesNamespace["play_group_command"] = obe::Config::Templates::play_group_command;
    }
    void LoadGlobalSetAnimationCommand(sol::state_view state)
    {
        sol::table TemplatesNamespace = state["obe"]["Config"]["Templates"].get<sol::table>();
        TemplatesNamespace["set_animation_command"] = obe::Config::Templates::set_animation_command;
    }
};