...
namespace obe::Collision
{
    ...
    convexHull(std::vector<Transform::UnitVector> points)
    {
        ...
        for (Transform::UnitVector& point : points)
        {
            while (lowerHull.size() >= 2 &&
                   cross(lowerHull[lowerHull.size() - 2],
        ...
        for (Transform::UnitVector& point : points)
        {
            while (upperHull.size() >= 2 &&
                   cross(upperHull[upperHull.size() - 2],
                         upperHull[upperHull.size() - 1], point) <= 0)
                upperHull.pop_back();
    ...
    PolygonalCollider::retrieveTagVector(ColliderTagType tagType) const
    {
        switch (tagType)
        {
    ...
    std::vector<std::string>&
    PolygonalCollider::retrieveTagVector(ColliderTagType tagType)
    {
        switch (tagType)
        {
        case ColliderTagType::Tag:
        ...
        const Transform::UnitVector& offset) const
    {
        ...
        {
            std::pair<double, Transform::UnitVector> minDist(
                -1, Transform::UnitVector());
    ...
    void PolygonalCollider::draw(Scene::Camera& camera, bool drawLines,
                                 bool drawPoints, bool drawMasterPoint,
                                 bool drawSkel)
    {
        if (m_points.size() >= 3)
        {
            ...
            for (int i = 0; i < m_points.size(); i++)
            {
                ...
                if (Utils::Vector::contains(i, m_highlightedPoints) &&
                    m_selected)
                    drawOptions["point_color_" + std::to_string(i)] =
                        sf::Color(255, 0, 0);
                if (Utils::Vector::contains((i != m_points.size() - 1) ? i + 1
                                                                       : 0,
                                            m_highlightedLines) &&
                    m_selected)
                    drawOptions["line_color_" + std::to_string(i)] =
                        sf::Color(0, 255, 0);

                lDrawPoints.emplace_back(point.x - offset.x,
                                         point.y - offset.y);
            }
            ...
            if (drawMasterPoint)
            {
                ...
                polyPt.setPosition(sf::Vector2f(pMaster.x - offset.x - r,
                                                pMaster.y - offset.y - r));
                polyPt.setRadius(r);
                const sf::Color polyPtColor = m_selected
                                                  ? sf::Color(0, 150, 255)
                                                  : sf::Color(255, 150, 0);
                ...
                if (drawSkel)
                {
                    for (int i = 0; i < m_points.size(); i++)
                    {
                        ...
                        const sf::Color currentLineColor =
                            m_selected ? sf::Color(0, 200, 255)
                                       : sf::Color(255, 200, 0);
                        Graphics::Utils::drawLine(
                            point.x - offset.x, point.y - offset.y,
                            pMaster.x - offset.x, pMaster.y - offset.y, 2,
                            currentLineColor);
                    }
                }
            }
    ...
    void PolygonalCollider::removeOriginChild(PolygonalCollider* child,
                                              bool trigger)
    {
    ...
    void PolygonalCollider::addTag(ColliderTagType tagType,
    ...
    bool
    PolygonalCollider::doesCollide(const Transform::UnitVector& offset) const
    {
        bool collided = false;
        for (auto& collider : Pool)
        {
    ...
    void PolygonalCollider::removeTag(ColliderTagType tagType,
                                      const std::string& tag)
    {
        Utils::Vector::eraseAll(this->retrieveTagVector(tagType), tag);
    }

    bool PolygonalCollider::doesHaveTag(ColliderTagType tagType,
                                        const std::string& tag)
    {
    ...
    std::vector<std::string>
    PolygonalCollider::getAllTags(ColliderTagType tagType) const
    {
        ...
        PolygonalCollider& collider, const Transform::UnitVector& offset) const
    {
        ...
        const auto calcMinDistanceDep =
            [this](const Transform::PolygonPath& sol1,
            ...
            for (auto& currentPoint : sol1)
            {
                ...
                for (int i = 0; i < sol2.size(); i++)
                {
                    ...
                    point3 =
                        sol2[(i == sol2.size() - 1) ? 0 : i + 1]->to(pxUnit);

                    s1 = point1 - point0;
                    s2 = point3 - point2;

                    s = (-s1.y * (point0.x - point2.x) +
                         s1.x * (point0.y - point2.y)) /
                        (-s2.x * s1.y + s1.x * s2.y);
                    t = (s2.x * (point0.y - point2.y) -
                         s2.y * (point0.x - point2.x)) /
                        (-s2.x * s1.y + s1.x * s2.y);
                    ...
                    if (s >= 0 && s <= 1 && t >= 0 && t <= 1)
                    {
                        inFront = true;
                        ip = point0 +
                             (s1 * Transform::UnitVector(t, t, s1.unit));
                        ...
                        if (distance < minDistance || minDistance == -1)
                        {
                            ...
                            double xComp = t * s1.x;
                            double yComp = t * s1.y;
                            minDeplacement.set((xComp > 0) ? std::floor(xComp)
                                                           : std::ceil(xComp),
                                               (yComp > 0) ? std::floor(yComp)
                                                           : std::ceil(yComp));
                        }
                    }
                }
        ...
        auto tdm2 = calcMinDistanceDep(
            sPath, fPath,
            tOffset * Transform::UnitVector(-1.0, -1.0, tOffset.unit));
    ...
    bool
    PolygonalCollider::doesCollide(PolygonalCollider& collider,
                                   const Transform::UnitVector& offset) const
    {
        ...
        for (auto& applyOffset : pSet1)
            applyOffset += offset;
        const auto pointInPolygon =
            [](const std::vector<Transform::UnitVector>& poly,
    ...
    {
        if (this->doesHaveAnyTag(ColliderTagType::Rejected,
                                 collider.getAllTags(ColliderTagType::Tag)))
            return false;
        if (!m_acceptedTags.empty() &&
            !this->doesHaveAnyTag(ColliderTagType::Accepted,
                                  collider.getAllTags(ColliderTagType::Tag)))
            return false;
        return true;
    }
