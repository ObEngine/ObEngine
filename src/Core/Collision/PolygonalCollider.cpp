...
namespace obe::Collision
{
        ...
        if (m_allPoints.size() > 0)
        {
            ...
            if (neighboor)
            {
                ...
                if (rightNeighbor >= m_allPoints.size())
                    rightNeighbor = 0;
                const int leftNeighborDist = std::sqrt(std::pow((position.x - m_allPoints[leftNeighbor].x), 2) +
                    std::pow((position.y - m_allPoints[leftNeighbor].y), 2));
                const int rightNeighborDist = std::sqrt(std::pow((position.x - m_allPoints[rightNeighbor].x), 2) +
    ...
    unsigned int PolygonalCollider::findClosestLine(const Transform::UnitVector& position)
    {
        ...
        {
            const Transform::UnitVector p1 = m_allPoints[i];
            const Transform::UnitVector p2 = m_allPoints[j];
            const double currentDistance = distanceLineFromPoint(p3, p1, p2);
    ...
    int PolygonalCollider::getSideContainingPoint(const Transform::UnitVector& position)
    {
        ...
        {
            const int nextNode = (i != m_allPoints.size() - 1) ? i + 1 : 0;
            const double lineLength = this->getDistanceFromPoint(i, m_allPoints[nextNode]);
            const double firstLength = this->getDistanceFromPoint(i, position);
            const double secondLength = this->getDistanceFromPoint(nextNode, position);
    ...
    Transform::UnitVector PolygonalCollider::getMaximumDistanceBeforeCollision(const Transform::UnitVector& offset) const
    {
        ...
        const Transform::UnitVector destPos = (m_masterPoint + offset).to<Transform::Units::WorldPixels>();
    ...
    void PolygonalCollider::movePoint(const unsigned int index, const Transform::UnitVector& vec)
    {
    ...
    void PolygonalCollider::setPointPositionFromMaster(const unsigned int index, const Transform::UnitVector& vec)
    {
        m_allPoints[index] = vec.to<Transform::Units::WorldUnits>() + m_masterPoint;
        calculateMasterPoint();
    }

    double PolygonalCollider::getSideAngle(const unsigned int side)
    {
        const int p1 = side;
        int p2 = side + 1;
        if (p1 == m_allPoints.size() - 1)
            p2 = 0;
        const Transform::UnitVector p1coords = this->getPointPosition(p1);
        const Transform::UnitVector p2coords = this->getPointPosition(p2);
        const double deltaX = p2coords.x - p1coords.x;
        const double deltaY = p2coords.y - p1coords.y;
        return (atan2(deltaY, deltaX) * 180.0 / Utils::Math::pi);
    }

    double PolygonalCollider::getSideLength(const unsigned int side)
    {
        const int p1 = side;
        int p2 = side + 1;
        if (p1 == m_allPoints.size() - 1)
    ...
    void PolygonalCollider::draw(Scene::Camera& camera, bool drawLines, bool drawPoints, bool drawMasterPoint, bool drawSkel)
    {
        if (m_allPoints.size() >= 3)
        {
            ...
            const Transform::UnitVector pMaster = m_masterPoint.to<Transform::Units::WorldPixels>();
            ...
            if (drawMasterPoint)
            {
                ...
                polyPt.setRadius(r);
                const sf::Color polyPtColor = m_selected ? sf::Color(0, 150, 255) : sf::Color(255, 150, 0);
    ...
    Transform::UnitVector PolygonalCollider::getMaximumDistanceBeforeCollision(const PolygonalCollider& collider, const Transform::UnitVector& offset) const
    {
        ...
        const auto calcMinDistanceDep = [this](PolygonPath& sol1, PolygonPath& sol2, const Transform::UnitVector& tOffset) -> std::tuple<double, Transform::UnitVector, bool>
        {
    ...
    bool PolygonalCollider::doesCollide(const PolygonalCollider& collider, const Transform::UnitVector& offset) const
    {
        ...
        for (Transform::UnitVector& applyOffset : pSet1)
            applyOffset += offset;
        const auto pointInPolygon = [](const PolygonPath& poly, Transform::UnitVector& pTest) -> bool
        {
            int i, j, c = 0;
