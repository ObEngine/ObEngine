    ...
    void TrajectoryNode::update(const double dt)
    {
        auto getOffset = [&dt](Trajectory& trajectory) {
            ...
            const double radAngle =
                (Utils::Math::pi / 180.0) * -trajectory.getAngle();
        ...
        for (auto& trajectory : m_trajectories)
        {
            ...
            if (cTraj->isEnabled())
            {
                ...
                if (!cTraj->getStatic())
                {
                    ...
                    cTraj->setSpeed(cTraj->m_speed +
                                    cTraj->m_acceleration * dt); // Recalculated here since it could change during the check
                    baseOffset = getOffset(*cTraj);
                    Transform::UnitVector realOffset = baseOffset;
                    if (m_probe != nullptr)
                    {
                        realOffset = m_probe->getMaximumDistanceBeforeCollision(
                            realOffset);
