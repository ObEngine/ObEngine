...
namespace obe::Graphics::Utils // <REVISION> Move to Utils/ ?
{
    ...
    const Value& get_or(const std::unordered_map<Key, Value>& m, const Key& key, const Value& default_value)
    {
        auto it = m.find(key);
        if (it == m.end())
        {
        ...
        const DrawPolygonOptions& options)
    {
        ...
        for (std::size_t i = 0; i < points.size(); i++)
        {
            ...
            if (options.lines)
            {
                const Color currentLineColor = get_or(options.specificLineColor, static_cast<unsigned int>(i), options.lineColor);
        ...
        for (std::size_t i = 0; i < points.size(); i++)
        {
            ...
            if (options.points)
            {
                const Color currentPointColor = get_or(options.specificPointColor,
                    static_cast<unsigned int>(i), options.pointColor);
