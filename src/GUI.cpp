...
GUI::LoadingBar::LoadingBar(std::string ID, int posX, int posY, std::string fillingType, std::string style, int sideBordersWidth, int TopBotBordersHeight) : Widget(ID, posX, posY, style)
{
	...
	selector.push_back(Display::Sprite);
	attributes->at(this->ID)->createBaseAttribute("type", widgetType);
}
...
void GUI::Button::setLabelText(GUI::Label* text)
{
	if (hasText)
	{
		...
		attributes->at(this->ID)->deleteComplexAttribute("containedItem", true);
	}
...
void GUI::Widget::addContainedItem(vili::ComplexAttribute* containedItem)
{
	...
	{
		attributes->at(ID, "containedItem")->pushComplexAttribute(containedItem->getComplexAttribute(*ite));
	}
}
