...
namespace obe
{
	namespace Functions
	{
		...
		bool Math::isDoubleInt(const double& value)
		{
		...
		bool Run::Parser::argumentExists(const std::string& arg) const
		{
			return std::find(start, start + size, arg) != (start + size);
		}

		std::string Run::Parser::getArgumentValue(const std::string& arg) const
		{
		...
		//Functions::String
		void String::removeCharFromString(std::string& str, const std::string& charToRemove)
		{
		...
		std::vector<std::string> String::multiSplit(std::string str, const std::vector<std::string>& seps)
		{
		...
		bool String::isStringInt(const std::string& str)
		{
			...
			{
				if (str.substr(0, 1) == "-")
					return std::all_of(str.substr(1).begin(), str.substr(1).end(), ::isdigit);
		...
		bool String::isStringFloat(const std::string& str)
		{
			...
			if (modifyStr.size() > 0)
			{
				...
				if (String::occurencesInString(modifyStr, ".") == 1)
				{
					isFloat = true;
					String::removeCharFromString(modifyStr, ".");
				}
				return (std::all_of(modifyStr.begin(), modifyStr.end(), ::isdigit) && isFloat);
			}
			else
				return false;
		}
