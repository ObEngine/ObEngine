...
namespace obe
{
    namespace Graphics
    {
        ...
        void TextRenderer::createRenderer(std::string rendererType, std::string id)
        {
            ...
            returnRenderer->load();
            returnRenderer->setFadeState(FadeIn);
        ...
        {
            if (rendererCalls.size() >= 1 && currentRenderer->getFadeState() != FadeOut)
                currentRenderer->setFadeState(FadeOut);
        }
        ...
        void TextRenderer::update(double dt)
        {
            if (currentRenderer != nullptr)
            {
                ...
                if (currentRenderer->getFadeState() == End && rendererCalls.size() > 1)
                {
                    rendererCalls.erase(rendererCalls.begin());
                    needToRender = true;
                    currentRenderer->setFadeState(FadeIn);
                    currentRenderer = rendererDB[rendererCalls[0]];
                }
                else if (currentRenderer->getFadeState() == End && rendererCalls.size() == 1)
                {
                    rendererCalls.erase(rendererCalls.begin());
                    currentRenderer->setFadeState(FadeIn);
                    currentRenderer = nullptr;
        ...
        void TextRenderer::render(sf::RenderWindow* surf)
        {
            ...
            {
                if (currentRenderer->getFadeState() == FadeIn)
                    currentRenderer->fadeIn(surf);
                else if (currentRenderer->getFadeState() == Draw)
                    currentRenderer->draw(surf);
                else if (currentRenderer->getFadeState() == FadeOut)
        ...
        void Renderers::Choice::render()
        {
            ...
            int selectedAnswer = stoi(vtdb[0]["selectedAnswer"]);
