...
namespace obe::transform
{
    ...
    class PolygonPoint : public Vector2
    {
    private:
    ...
    public:
        ...
        explicit PolygonPoint(
            Polygon& parent, point_index_t index, const transform::Vector2& position);

        PolygonPoint& operator=(const PolygonPoint& point);

        const point_index_t& index = rw_index;
        void remove() const;
        [[nodiscard]] double distance(const transform::Vector2& position) const;
        [[nodiscard]] Vector2 get_relative_position(RelativePositionFrom from) const;
        void set_relative_position(RelativePositionFrom from, const transform::Vector2& position);
    ...
    public:
         ...
         */
        PolygonSegment find_closest_segment(const transform::Vector2& position);
        /**
         ...
         */
        PolygonPoint& find_closest_point(const transform::Vector2& position, bool neighbor = false,
         ...
         * \return An Vector2 containing the position of the Master Point
         *         (centroid) of the Polygon
         */
        [[nodiscard]] transform::Vector2 get_centroid() const;
        /**
         ...
         * \return An Vector2 containing the position of the first point of
         * the Polygon
         */
        ...
        std::optional<PolygonSegment> get_segment_containing_point(
            const transform::Vector2& position, double tolerance = DefaultTolerance);
        /**
         ...
         * \param position Vector2 containing the offset to move the Polygon
         */
        void move(const transform::Vector2& position);
        /**
         * \brief Sets the new position of the Polygon (using the point at index 0)
         * \param position Vector2 containing the new Position of the
         *        Polygon
         */
        void set_position(const transform::Vector2& position);

        virtual void set_position_from_centroid(const transform::Vector2& position);
         ...
         */
        virtual void set_rotation(float angle, transform::Vector2 origin);
        /**
         ...
         * \param origin Origin to rotate all the points around
         */
        virtual void rotate(float angle, transform::Vector2 origin);
        /**
