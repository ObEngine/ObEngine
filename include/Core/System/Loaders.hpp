...
namespace obe::System::Loaders
{
    ...
    private:
        ...
        std::string path;
    public:
        NonOwnedLoaderResult(const LoaderType<ResourceType>& loader, LoadedResource& reference, const std::string& path);
        bool load();
    ...
    private:
        ...
        std::string path;
    public:
        LoaderResult(const LoaderType<ResourceType>& loader, const std::string& path);
        NonOwnedLoaderResult<LoaderType<ResourceType>>& use(ResourceType& resource);
        template <class ...Args>
    ...
    */
    ...
    inline NonOwnedLoaderResult<LoaderType<ResourceType>>::NonOwnedLoaderResult(const LoaderType<ResourceType>& loader, LoadedResource& reference, const std::string& path)
        : m_non_owned_resource(reference), m_loader(loader)
    {
        m_path = path;
    }
    ...
    template <class LoaderType<class ResourceType>>
    inline std::string NonOwnedLoaderResult<LoaderType<ResourceType>>::getPath() const
    {
        return m_path;
    }

    template <class LoaderType<class ResourceType>>
    inline LoaderResult<LoaderType<ResourceType>>::LoaderResult(const LoaderType<ResourceType>& loader, const std::string& path) : m_loader(loader)
    {
        m_path = path;
    }

    template <class LoaderType<class ResourceType>>
    inline NonOwnedLoaderResult<LoaderType<ResourceType>>& LoaderResult<LoaderType<ResourceType>>::use(LoadedResource& resource)
    {
        return NonOwnedLoaderResult<LoadedResource, LoaderClass>(resource, m_path);
    }

    template <class LoaderType<class ResourceType>>
    template <class ...Args>
    inline std::optional<std::unique_ptr<ResourceType>> LoaderResult<LoaderType<ResourceType>>::load(Args... args)
    {
        std::unique_ptr<LoadedResource> newResource = std::make_unique<LoadedResource>(std::forward(args...));
        if (m_loader.load(&newResource, m_path))
        {
            return std::make_optional<std::unique_ptr<LoadedResource>>(std::move(newResource));
        }
        else
        {
    ...
    template<class Resource>
    inline Loader<Resource>::Loader(const std::function<bool(Resource&, const std::string&)>& function)
    {
        m_function = function;
    }
