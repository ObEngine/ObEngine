...
namespace vili
{
    LinkAttribute::LinkAttribute(ComplexAttribute* parent, const std::string& id, const std::string& path) : Attribute(parent, id, AttributeType::LinkAttribute)
    {
        m_path = path;
    }
    ...
    Attribute* LinkAttribute::getTarget()
    {
        ...
        if (complexParent->contains(AttributeType::BaseAttribute, "__linkroot__"))
        ...
        {
            if (location->getType() == AttributeType::ComplexAttribute)
            {
                ComplexAttribute* complexLocation = static_cast<ComplexAttribute*>(location);
                if (complexLocation->contains(AttributeType::ComplexAttribute, pathPart))
                    location = &complexLocation->getComplexAttribute(pathPart);
                else if (complexLocation->contains(AttributeType::BaseAttribute, pathPart))
                {
                    location = &complexLocation->getBaseAttribute(pathPart);
                    break;
                }
                else if (complexLocation->contains(AttributeType::ListAttribute, pathPart))
            ...
            else if (location->getType() == AttributeType::ListAttribute)
            {
    ...
    void LinkAttribute::apply()
    {
        ...
        else if (getTarget()->getType() == AttributeType::ListAttribute)
            dynamic_cast<ListAttribute*>(getTarget())->copy(complexParent, m_id);
        else if (getTarget()->getType() == AttributeType::BaseAttribute)
            dynamic_cast<BaseAttribute*>(getTarget())->copy(complexParent, m_id);
        else if (getTarget()->getType() == AttributeType::LinkAttribute)
