...
namespace TAO_PEGTL_NAMESPACE
{
    namespace internal
    {
        ...
        inline constexpr bool raise_on_failure
            = (Errors::template message<Rule> != nullptr);

        template <typename Errors, typename Rule>
        inline constexpr bool raise_on_failure<Errors, Rule,
            std::void_t<decltype(Errors::template raise_on_failure<Rule>)>> = Errors::
            template raise_on_failure<Rule>;

    } // namespace internal

    template <typename Errors, template <typename...> class Base = normal,
    ...
    struct must_if
    {
        template <typename Rule> struct control : Base<Rule>
        {
            ...
            static void failure(const ParseInput& in, States&&... st) noexcept(
                noexcept(Base<Rule>::failure(in, st...))
            ...
            [[noreturn]] static void raise(
                const ParseInput& in, [[maybe_unused]] States&&... st)
            {
