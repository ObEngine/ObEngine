...
double ExpOperator::processResult() {
	if (op->content != "@")
	{
		...
		if (opMode == 3)
		{
			if (fn::String::isStringNumeric(leftSimpleOp->content))
				leftMember = std::stod(leftSimpleOp->content);
			else
				leftMember = varBaseHook->getVar(leftSimpleOp->content);
			if (fn::String::isStringNumeric(rightSimpleOp->content))
	...
	else
	{
		...
		std::vector<std::string> multipleParameter = fn::String::split(leftSimpleOp->content, ",");
...
MathExp::MathExp(int polymorph, std::string file, bool strict)
{
	mathexpFile.open("Data/Curves/" + file + ".msmxp");
	std::string fileBuffer;
	mathexp = "";
	if (mathexpFile.is_open())
	{
		while (std::getline(mathexpFile, fileBuffer))
			mathexp += fileBuffer;
	}	
	this->varBase = new VarDatabase;
	this->varBase->setStrict(strict);
